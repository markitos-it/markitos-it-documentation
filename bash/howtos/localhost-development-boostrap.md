## Automatización del Entorno DevSecOps
### Arquitectura y Componentes del Sistema

Al profundizar en la lógica operativa que ejecuta este automatismo, cada módulo instalado cumple un propósito estratégico para garantizar la inmutabilidad y eficiencia de los flujos de integración y despliegue continuo:

* **Gestores de Versiones Aislados**: El uso de `tfenv`, `goenv` y `nvm` es un principio fundamental para arquitecturas distribuidas. En lugar de instalar binarios a nivel de sistema que puedan entrar en conflicto entre distintos repositorios, estos gestores permiten que cada microservicio declare la versión exacta de Go o Terraform que requiere. Esto evita por completo la "deriva de código" y asegura que lo que funciona en local funcionará exactamente igual en los clústeres.
* **Google Cloud SDK Integral**: La incorporación del componente `gcloud` directamente en el `PATH` asegura un acceso ágil a la gestión de recursos de nube. Esto resulta indispensable para aplicar de forma programática las políticas de IAM, gestionar identidades de carga de trabajo y provisionar infraestructura subyacente.
* **Idempotencia y Resiliencia**: El diseño del script evalúa dinámicamente el estado del entorno antes de realizar modificaciones. Si una herramienta ya se encuentra disponible en el sistema, actualiza sus dependencias y binarios sin duplicar descargas, manteniendo el sistema libre de artefactos residuales y optimizando los tiempos de inicialización.

### Guía de Despliegue Rápido

Para integrar estas herramientas en el entorno de trabajo, sigue estos pasos:

1. Guarda el código superior en un archivo local llamado `bootstrap.sh`.
2. Aplica permisos de ejecución mediante el comando `chmod +x bootstrap.sh`.
3. Para una instalación interactiva ejecuta `./bootstrap.sh`. Si se prefiere un despliegue desatendido en sistemas efímeros, añade la bandera `./bootstrap.sh --all-yes`.


A continuación tienes el script formateado y listo para ser utilizado.
